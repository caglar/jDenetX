...
public class DoubleVector extends AbstractMOAObject {
    ...
    public void addExpWeightedValues(DoubleVector toAdd, double[] errors) {

        if (toAdd.getArrayRef().length > this.array.length) {
            setArrayLength(toAdd.getArrayRef().length);
        }
        for (int i = 0; i < toAdd.getArrayRef().length; i++) {
            this.array[i] += toAdd.getValue(i);
        }
    }
    ...
    public double sumOfValues() {
        double sum = 0.0;
        for (int i = 0; i < this.array.length; i++) {
//            System.out.println("Element is: " + this.array[i]);
            sum += this.array[i];
        }
//        System.out.println("Sum is: " + sum);
    ...
    public void normalize() {
        double sumOfVals = sumOfValues();
        if (sumOfVals != 0) {
            scaleValues(1.0 / sumOfVals);
        }
    }
